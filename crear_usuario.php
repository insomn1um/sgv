...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $usuario->creado_por = $_SESSION['user_id'];

    if (!$usuario->username || !$usuario->nombre || !$usuario->apellido || !$usuario->email || !$usuario->rol || !$usuario->password) {
        $error = 'Todos los campos son obligatorios.';
    } elseif ($usuario->usernameExists($usuario->username)) {
        $error = 'El nombre de usuario ya existe.';
    } elseif ($usuario->emailExists($usuario->email)) {
        $error = 'El email ya está registrado.';
    } else {
        $usuario_id = $usuario->create();
        if ($usuario_id) {
            logAuditoria('crear', 'usuarios', $usuario_id, null, json_encode([
...
<head>
    ...
    <style>
        /* Estilos específicos para crear_usuario.php si son necesarios */
    </style>
</head>
<body>
    <!-- Navbar -->
                    ...
                    <div class="col-lg-8">
                        <div class="card shadow-lg">
                            <div class="card-header " style="background-color: #3b82f6; color: white;">
                                <h4 class="mb-0"><i class="fas fa-user-plus"></i> Información del Usuario</h4>
                            </div>
                            <div class="card-body">
                                <?php if ($error): ?>
