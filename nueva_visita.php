...
<body>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="" id="visitaForm" class="needs-validation" novalidate data-auto-datetime="<?php echo empty($_POST) ? '1' : '0'; ?>">
                                <div class="row">
                                    <!-- Columna Izquierda: Información del Visitante (se llena primero) -->
                                    <div class="col-md-6">
                                        ...
                                        <div class="mb-3">
                                            <label for="buscar_visitante" class="form-label">
                                                <i class="fas fa-search"></i> Buscar visitante anterior
                                            </label>
                                            <div class="position-relative">
                                                <input type="text" id="buscar_visitante" class="form-control" 
                                                       placeholder="Escriba nombre, apellido o RUT para sugerencias...">
                                                <div id="sugerencias_visitante" class="list-group position-absolute top-100 start-0 end-0 mt-1 shadow d-none" 
                                                     style="z-index: 1050; max-height: 240px; overflow-y: auto;"></div>
                                            </div>
                                            <div class="help-text">Seleccione una sugerencia para cargar los datos automáticamente</div>
                                        </div>
    ...
    <script>
        ...
        // Establecer fecha y hora actual del dispositivo al cargar el formulario
        document.addEventListener('DOMContentLoaded', function() {
            var form = document.getElementById('visitaForm');
            if (form && form.dataset.autoDatetime === '1') {
                var now = new Date();
                var d = document.getElementById('fecha_ingreso');
                var h = document.getElementById('hora_ingreso');
                if (d && h) {
        ...
        // Autocompletar visitante anterior
        document.addEventListener('DOMContentLoaded', function() {
            var busqueda = document.getElementById('buscar_visitante');
            var sugerencias = document.getElementById('sugerencias_visitante');
            var debounceTimer;
            
            if (!busqueda || !sugerencias) return;
            
            function llenarFormulario(r) {
                var f = function(id, val) { var e = document.getElementById(id); if (e) e.value = val || ''; };
                f('nombre_visitante', r.nombre);
                f('apellido_visitante', r.apellido);
                f('tipo_identificacion', r.tipo_identificacion);
                f('numero_identificacion', r.numero_identificacion);
                f('numero_contacto', r.numero_contacto);
                f('email', r.email);
                f('empresa_visitante', r.empresa_visitante);
                f('patente', r.patente);
                f('tipo_vehiculo', r.tipo_vehiculo);
                f('numero_tarjeta', r.numero_tarjeta);
                busqueda.value = '';
            ...
            busqueda.addEventListener('input', function() {
                ...
                debounceTimer = setTimeout(function() {
                    fetch('ajax/buscar_visitantes_sugerencia.php?q=' + encodeURIComponent(q))
                        .then(function(res) { return res.json(); })
                        .then(function(data) {
                            var list = data.resultados || [];
                            sugerencias.innerHTML = '';
                            list.forEach(function(r) {
                                var text = (r.nombre + ' ' + r.apellido).trim() + 
                                    (r.numero_identificacion ? ' - ' + r.numero_identificacion : '') +
                                    (r.empresa_visitante ? ' (' + r.empresa_visitante + ')' : '');
                                var a = document.createElement('a');
                                a.href = '#';
                                a.className = 'list-group-item list-group-item-action';
                                a.textContent = text || 'Sin nombre';
                            ...
                            sugerencias.classList.toggle('d-none', list.length === 0);
                        });
                }, 350);
            });
